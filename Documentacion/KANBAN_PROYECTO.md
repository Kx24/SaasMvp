...
>
> **Piloto agéntico:** `docs/kanban_agente.md`, branch `agent/ai-dlc-pilot`, worktree `SaaSMVP-agentic-pilot` — toma cards de plataforma de este kanban 100% ejecutables sin secretos/insumos externos y las ejecuta desatendido (WIP=1, regla §0.4 ahí: nunca toca nada de Rancho Cachimba). No reemplaza este archivo; su §4 (registro de ejecución) es la fuente de qué está DONE ahí y todavía sin mergear a `develop`/`main`/esta branch — este archivo se sincroniza contra ese registro a mano, no automáticamente.

---

## 🌙 Retomar aquí (actualizado 2026-08-24)

**`agent/ai-dlc-pilot` mergeado a `develop` y pusheado a GitHub (2026-08-24).** Las 9 cards `BOLT-01..09` (ver nota del piloto más abajo) ya están en `develop` — commit de merge `45eca7b`, gate re-verificado ahí: 162 tests OK (1 skip), `ruff` limpio en los archivos que trajo, `makemigrations --check` limpio. Este archivo se reconcilió con la versión más completa (traída de `feature/RanchocachimbaEtapa1`) en el mismo commit.

**`#DEUDA-03` cerrado en `develop` (2026-08-24) — encontró un bug real en el checkout pagado, no solo la inconsistencia de carpetas que decía la card original.** El Plan Pro (destacado, $250.000) ofrecía temas (`'electricidad'`/`'industrial'`) que no coincidían con ninguna carpeta real de `templates/`; un cliente que pagaba y los elegía terminaba con el sitio roto (fallback global genérico). Detalle completo, decisión del usuario (recortar a temas reales en vez de reactivar el tema huérfano) y verificación (suite + Playwright) en la card `#DEUDA-03` de §6.

**`#DS-03` cerrado en `develop` (2026-08-24, commit `c875540`) — gate real de accesibilidad AA con `@axe-core/playwright`, no una auditoría manual.** Rojo confirmado (17+3+9 violaciones reales en servelec/andesscale/login) antes del fix; corregidas todas — 0 violaciones WCAG2A/AA en las 3 páginas reales de `develop`. Detalle completo (incluido el hallazgo de que `.fade-up` producía falsos positivos, y el de `#1DB954` hardcodeado en servelec ignorando la marca real del tenant) en la card `#DS-03` de §6.

**`#DEUDA-02` Fase 1 cerrada en `develop` (2026-08-24, commit `1a0794d`) — usó `EnterPlanMode` por ser `[XL]` con "no empezar a medias" explícito en la card.** `GalleryItem.gallery_type` (2 choices fijas) reemplazado por FKs `section`/`service` (no M2M); el rol de una imagen sale de a qué está linkeada, no de un enum propio. Capacidad nueva: un `Service` puede tener galería de varias fotos. **Fase 2 (migrar `Section.image`/`background_image`/`Service.image` al pool) queda explícitamente fuera** — decisión del usuario tras ver el alcance real (~16 templates, forms del dashboard, cero tests hoy). Detalle completo en la card `#DEUDA-02` de §6.

**`#SEC-03` cerrado en `develop` (2026-08-24, commit `5229215`) — ⚠️ hallazgo urgente, acción pedida al usuario:** `setup_production.py` creaba el primer superusuario de `/superadmin/` con la contraseña hardcodeada `admin123456` si `DJANGO_SUPERUSER_PASSWORD` nunca se seteó en Render (esa env var no estaba ni declarada en `render.yaml`). El código ya se corrigió (falla el build en vez de crear la cuenta débil), pero **si el superuser actual de producción se creó antes de este fix, su contraseña podría seguir siendo la default** — verificar/rotar en `/superadmin/` cuanto antes. Detalle completo y el resto del inventario de secretos (MercadoPago/Cloudinary tampoco estaban declarados en `render.yaml`) en `Documentacion/Inventario_Secretos.md` y la card `#SEC-03` de §5.

**`#FLOW-01` cerrado en `develop` (2026-08-24, commit `2025343`) — encontró que `provision_tenant` (el único camino de alta manual) estaba roto de la misma forma que `#DEUDA-03` encontró en el checkout: un flag mezclaba industria con tema visual y el tenant caía en el fallback sin marca, sin error visible.** El fix ya existía en `feature/RanchocachimbaEtapa1` (2026-08-10) pero nunca llegó a `develop`; se portó adaptado. De paso, `list_tenants`/`update_domain` tenían el mismo bug de `#DEUDA-03`/`#AUD-05` (referenciar un campo que no existe) — el segundo sobreescribía el dominio de TODOS los clientes. Nuevo runbook canónico en `Documentacion/Procedimiento_Nuevo_Tenant.md`, verificado en vivo contra un tenant real. Detalle completo en la card `#FLOW-01`/`#FLOW-02`/`#PAY-02` de §5.
...
**Piloto agéntico (`agent/ai-dlc-pilot`, worktree `SaaSMVP-agentic-pilot`) — verificado contra el repo 2026-08-24, trabajo sin mergear a ninguna otra branch.** Las 9 cards de plataforma `BOLT-01..09` (`docs/kanban_agente.md` §2, esa branch) están **DONE (2026-08-23)**, confirmado con `git log`/`git ls-tree` — ninguna llegó todavía a `develop`, `main` ni a esta branch. En la práctica cierran: el checkbox pendiente de `#AUD-01` (slugs reservados `process`/`success`/`error` en `Plan`, `BOLT-02`), las dos mitades de `#MED-05` (IP confiable tras el proxy `BOLT-03`, rate limit en login/checkout `BOLT-04`), la ampliación de `#FLOW-02`/`check_tenant_setup` como gate (`BOLT-05`), la mitad automatizable de `#MED-04` (texto plano legible + link de soporte roto, `BOLT-09`), y las 3 generalizaciones de plataforma del análisis de `#RC-20` — guardia del contrato de tokens CSS, CTA de navbar configurable por tenant, `hero_ctas` a componente compartido (`BOLT-06..08`, detalle en la card `#RC-20` más abajo). **Deuda de integración confirmada, no es solo del piloto:** verificado con `git ls-tree` que `develop`/`main` tampoco tienen `apps/tenants/management/commands/check_tenant_setup.py` ni el retiro de `apps/core/managers.py` (código muerto) que esta branch cerró en `#DEUDA-05` (commit `d3bb7ba`, solo acá) — el piloto encontró el mismo hallazgo de forma independiente y lo volvió a cerrar en `BOLT-01` porque no le llegaba desde `develop`. `.claude/skills/andesscale-saas/SKILL.md` sí está commiteado en las 3 branches (`develop`/`main`/esta) — el hallazgo de `PILOT-02` (que decía que nunca se había commiteado en ninguna rama) fue correcto en su momento, pero ya se resolvió: `develop`/`main` lo recibieron por commits directos (`b58f5fe`/`fad374d`, "nunca se había commiteado") apenas 13 minutos antes de que esta branch cerrara su propio `#DEUDA-05` (`d3bb7ba`) — ese hallazgo de `docs/kanban_agente.md` §3 ya está desactualizado. **Decisión de merge/cherry-pick entre las 3 branches queda para el usuario** — no se tocó código ni se mergeó nada en esta sesión, solo se sincronizó este kanban con lo verificado en el repo.
...
1. ~~`#AUD-01` — falta el caso "un plan no puede llamarse `process`/`success`/`error`".~~ **Resuelto en código** por `BOLT-02` del piloto (`Plan.clean()` rechaza los 3 slugs) — sin mergear a esta branch todavía, ver nota del piloto arriba.
...
**Lo que sigue, sin depender de Rancho Cachimba** (todas `[P2-Media]`, independientes entre sí, sin orden forzoso — ver §5 para el detalle de cada una): `#MED-04` (mitad manual: probar en Gmail/Outlook — la mitad automatizable ya está DONE en `agent/ai-dlc-pilot`, sin mergear), `#MED-05` (DONE en `agent/ai-dlc-pilot`, sin mergear), `#PAY-02` (registro de fricciones del cobro manual de Cachimba — `#FLOW-01`/`#FLOW-02`/`#SEC-03` ya DONE). El resto (`#RC-01`/`#RC-06b`/`#RC-18`) sigue en pausa por decisión del usuario, que necesita tiempo de diseño/investigación antes de retomarlo. También quedó postergado (a pedido del usuario) verificar `EXPLAIN ANALYZE` de `#MED-03` contra la base real — nota aparte: la base de producción es **Neon**, no Supabase como dice el resto de este documento; corrección pendiente, el usuario pidió posponer el tema completo hasta tener más contexto sobre Neon.
...
#### `#MED-04` — Plantillas HTML transaccionales responsive `[P2-Media]` `[M]` `[Frontend]` *(mitad automatizable DONE en `agent/ai-dlc-pilot`, sin mergear)*
**Mitad automatizable cerrada por `BOLT-09` (2026-08-23, commit `2593784`):** texto plano real por template (`emails/*.txt`, reemplaza `strip_tags` que dejaba el CSS de `<style>` como texto visible) + link de soporte del footer reparado (apuntaba a un artefacto de ofuscación de Cloudflare, muerto en el 100% de los casos) + `color-scheme`/`supported-color-schemes` para dark-mode. **Falta (bloqueado en `docs/kanban_agente.md` §3, requiere casillas reales del usuario):** probar en Gmail/Outlook móvil real.

#### `#MED-05` — Rate limit y auditoría de IP confiable `[P2-Media]` `[S]` `[Backend]` — **DONE en `agent/ai-dlc-pilot` (2026-08-23), sin mergear**
Tomar la IP del último proxy confiable (Render setea XFF); aplicar `RateLimiter` también a login (`scope='login'`) y checkout. Cerrado en dos partes por el piloto: `BOLT-03` (`f570cda`) unificó las 3 copias divergentes de `get_client_ip()` (`apps/core/rate_limit.py`, consumida por import en `orders`/`website`) tomando el valor a `TRUSTED_PROXY_COUNT` posiciones desde la derecha del XFF; `BOLT-04` (`9e8ab83`) agregó rate limit a login (5 intentos fallidos/5 min por IP+hash(username)+tenant, solo fallos consumen cupo) y checkout (10/10 min por IP+tenant, todo intento cuenta).
...
#### ✅ `#FLOW-01` / `#FLOW-02` / `#PAY-02` — Flujo comercial repetible `[P2-Media]` `[M]` — **`#FLOW-01`/`#FLOW-02` DONE, `#PAY-02` abierto**
Procedimiento de ingreso de cliente + `check_tenant_setup` ampliado (SEOConfig completo, sin placeholders, `ClientEmailSettings`) + registro de fricciones del cobro manual de Cachimba (es la spec de `#FLOW-03`). **`#FLOW-02`** (la ampliación de `check_tenant_setup`) DONE — llegó a `develop` vía el merge de `agent/ai-dlc-pilot` (`BOLT-05`). **`#FLOW-01` DONE (2026-08-24, en `develop`, commit `2025343`)** — al escribir el runbook apareció un bug real: `provision_tenant` (el único camino de alta manual, no-pago) tenía un solo flag `--template` que mezclaba industria (contenido semilla) con tema visual, y el valor guardado nunca coincidía con `Client.THEME_CHOICES` — cualquier tenant provisionado con este comando caía en el fallback genérico sin marca, sin ningún error visible. Este fix ya existía en `feature/RanchocachimbaEtapa1` (commit `03f14e2`, 2026-08-10) pero nunca llegó a `develop` — mismo patrón de integración pendiente del resto de la sesión; se portó adaptado al estado actual (`themes/servelec` post `#DEUDA-03`, campos reales de `ClientSettings`). De paso: `list_tenants` y `update_domain` referenciaban `Client.domain` (campo inexistente, vive en `Domain`) — el primero crasheaba siempre con algún tenant real, el segundo sobreescribía el dominio de TODOS los clientes con el mismo valor. `manage.py` fuerza UTF-8 en stdout/stderr (elimina la clase de `UnicodeEncodeError` que crasheaba varios management commands en consolas Windows). Eliminados `create_localhost_client.py`/`scriptsAislamiento.py` (código muerto, ya rotos por el mismo bug). Nuevo `Documentacion/Procedimiento_Nuevo_Tenant.md` (runbook canónico, adaptado del que ya existía en la branch de Rancho pero corregido contra la realidad actual: gate real de `check_tenant_setup` con exit code, SEO como `[FAIL]` bloqueante, `test_isolation` reemplazado por la suite, aclaración de que `localhost` es dominio de sistema). **Verificado en vivo** (no solo tests): secuencia completa `provision_tenant`→`check_tenant_setup`→`list_tenants`→`update_domain` corrida contra un tenant real, confirmando que el runbook documentado funciona tal cual. `apps/tenants/tests_provisioning.py` (10 tests nuevos), suite completa 195 tests OK (5 skip), gatekeeper en verde, `ruff`/`makemigrations --check` limpios. **`#PAY-02`** (registro de fricciones del cobro manual de Cachimba) sigue abierto — depende de insumos de la experiencia real con Rancho Cachimba, en pausa.
...
#### ✅ `#SEC-03` — Inventario de secretos `[P2-Media]` `[S]` `[DevOps]` — **DONE (2026-08-24, en `develop`, commit `5229215`)**
`.env` verificado fuera del historial ✅ (ya lo estaba). **Hallazgo urgente encontrado y corregido en el camino:** `setup_production.py` (corre en cada build) creaba el primer superusuario de `/superadmin/` con la contraseña hardcodeada `admin123456` si `DJANGO_SUPERUSER_PASSWORD` no estaba seteada — visible en el código fuente, y esa env var nunca estuvo declarada en `render.yaml` ni como recordatorio. El comando es idempotente (solo importa para el primer deploy de la base actual). Fix: mismo patrón fail-fast que `SECRET_KEY`/`EMAIL_HOST_USER` (`#AUD-07`/`#AUD-12`) — sin la env var, el build falla en vez de crear una cuenta con clave débil. **Acción pedida al usuario, no verificable desde acá:** entrar a `/superadmin/` en producción y confirmar que el superuser actual no tiene esa contraseña por defecto — ver el aviso completo en `Documentacion/Inventario_Secretos.md`. Segundo hallazgo: `render.yaml` nunca declaraba (ni como `sync: false`) las credenciales de MercadoPago ni Cloudinary pese a que el checkout y la subida de media las usan en producción — agregadas como `sync: false` (no cambia ningún valor real). `MP_SANDBOX` se agrega también pero deliberadamente sin fijar valor — pasarla a `False` habilita cobros reales, decisión de negocio que no le corresponde a este commit. **Resultado:** `.env.example` nuevo (no existía — el README ya instruía "cp .env.example .env", paso roto), `Documentacion/Inventario_Secretos.md` nuevo (tabla completa variable/sensibilidad/servicios + política de rotación por tipo), `apps/core/tests/test_render_config.py` extendido con las 10 keys nuevas (rojo confirmado vía `git stash`), `apps/tenants/tests_setup_production.py` (3 tests nuevos). Suite completa 198 tests OK (5 skip), gatekeeper en verde, `ruff`/`bandit`/`makemigrations --check` limpios. **Fuera de alcance, documentado:** confirmar qué está realmente seteado hoy en el dashboard de Render — requiere acceso que este documento no tiene (columna "¿Configurado en Render?" queda para completar a mano), mismo cabo suelto que `#AUD-04` ya tenía pendiente.
...
- 🟡 `#DEUDA-02` — **Pool único de imágenes** (`GalleryItem` sin `gallery_type` como rol fijo) `[P2]` `[XL]` `[Database]` — **Fase 1 DONE (2026-08-24, en `develop`, commit `1a0794d`)**, Fase 2 sin empezar. `gallery_type` (2 choices fijas) reemplazado por FKs nullable mutuamente excluyentes `section`/`service` (no M2M — ninguna imagen necesita hoy más de un dueño); el rol de una imagen sale de `section.section_type` (ya tiene 5 valores, no 2) o `role == 'service'`. `clean()` exige exactamente uno de los dos + mismo `client` que el dueño (guardia IDOR). Capacidad nueva: un `Service` puede tener galería de varias fotos (antes imposible, `Service.image` era un `CloudinaryField` único) — dashboard nuevo en `edit_service.html`, sin render público en ningún theme (decisión de diseño de frontend aparte). Investigación previa (3 agentes Explore) encontró que `provision_tenant.py` nunca crea una `Section(section_type='gallery')` ni ningún `GalleryItem` — `is_default` ("imagen seed") está muerto en la práctica; la migración de datos usa `get_or_create` para tolerarlo. Migraciones en 3 pasos (`0020` schema add nullable → `0021` data → `0022` schema remove) para no perder datos reales — aplicado y verificado contra `db.sqlite3` local (servelec/ranchocachimba/prueba). **Fase 2 explícitamente fuera de esta card** (decisión del usuario): migrar `Section.image`/`background_image`/`Service.image` (single-image `CloudinaryField`, no sufren el problema de "rol rígido") al pool — 16 templates + forms del dashboard, sin red de tests hoy. **Resultado:** `apps/website/tests/test_gallery_pool.py` (18 tests nuevos, rojo estructural confirmado), suite completa 185 tests OK (5 skip), `scripts/gatekeeper.py` en verde, `ruff` limpio, `makemigrations --check` limpio, `npx playwright test` 7/7. **Hallazgos incidentales corregidos en el camino:** `GalleryItem.get_background_image_url()` era código muerto copiado de `Section` (referenciaba un campo que `GalleryItem` no tiene); draft comentado de un `GalleryItem` viejo eliminado; `website_tags.py` llamaba `logger.warning()` sin logger definido (`NameError` latente); `gallery_item_delete`/`toggle` redirigían a una URL que `dashboard_gallery` ni lee para items de portada — centralizado en `_gallery_item_redirect_url()`.
- ✅ `#DEUDA-03` — **Unificar temas bajo `templates/themes/`** `[P2]` `[M]` — **DONE (2026-08-24, en `develop`)**. Al investigar apareció un bug real en el checkout pagado: el vocabulario de "tema" estaba roto en 3 lugares (`Client.THEME_CHOICES`, el `default` del campo `template`, y `Plan.available_themes`/`ClientOnboardingForm`) y ninguno coincidía con las carpetas reales de `templates/`. El **Plan Pro** ($250.000, destacado, `is_active=True`) ofrecía `'electricidad'`/`'industrial'` en el selector de onboarding; `apps/orders/views_onboarding.py::process_onboarding` crea el `Client` con `template=data['template']` sin `full_clean()`, así que un cliente que pagaba y elegía esas opciones terminaba con `TenantTemplateLoader` cayendo al fallback global genérico — sitio roto para un cliente que ya pagó. Fix: `THEME_CHOICES` recortado a `'themes/default'`/`'themes/servelec'` (únicos con carpeta real y mantenida, se retira `'themes/industrial'` que nunca tuvo carpeta); `default='themes/default'` (antes `'default'`, que no coincidía con nada); `templates/servelec/` → `templates/themes/servelec/` (`git mv` + 11 referencias internas del propio tema); `Plan.available_themes` recortado a los 2 temas reales para Pro/Enterprise (se retiran `'industrial'`/`'custom'`); `ClientOnboardingForm.theme_labels` con las claves reales. Migraciones `0024`+`0025` (tenants) y `0002` (orders) corrigen también los `Client`/`Plan` ya creados, no solo los futuros. **Resultado:** `apps/tenants/tests_theme_consistency.py` + `apps/orders/tests/test_plan_themes.py` (5 tests nuevos, rojo confirmado antes del fix), suite completa 167 tests OK (1 skip), `ruff` limpio, `makemigrations --check` limpio, `npx playwright test` 4/4 en verde (confirma en navegador real que `templates/themes/servelec/` resuelve end-to-end). `templates/themes/electricidad/` queda huérfana a propósito (sin `THEME_CHOICES` que apunte ahí) — decisión del usuario de no reactivarla hasta que exista un tema "Industrial" de verdad.
...
- ✅ `#DS-03` — **Accesibilidad AA** `[P2]` `[M]` — **DONE (2026-08-24, en `develop`, commit `c875540`)**. Gate nuevo con `@axe-core/playwright` (`tests/e2e/a11y.spec.js`) contra WCAG2A/AA en las 3 páginas reales de `develop` (home servelec, home andesscale, login compartido). **Rojo real confirmado antes del fix:** 17+3+9 violaciones (`color-contrast`, `link-name`, `link-in-text-block`). Hallazgo de método: la primera corrida reportó 38 violaciones en servelec — la mayoría falsos positivos por `.fade-up` (animación `opacity:0→1` de 0.6s que casi todas las secciones usan); con `page.waitForTimeout(1000)` antes de `analyze()` bajó a las 17 reales, verificadas nodo por nodo contra el HTML servido. Fixes: nuevas utilidades `.text-primary-a11y`/`.bg-primary-a11y` en `static/css/input.css` (mismo patrón `color-mix()` que ya usa `.btn-primary` — `var(--color-primary)` de un tenant no garantiza 4.5:1 tal cual, servelec verde `#16a34a` medía 2.6-3.3 en badges/botones/links reales) aplicadas en `templates/auth/login.html` (global), `templates/partials/contact_form.html` (compartido 3 temas) y 4 puntos de `templates/themes/servelec/` que hardcodeaban `#1DB954` en vez de `var(--color-primary)` — **hallazgo aparte, no corregido en su totalidad:** ese hardcodeo ignora `ClientSettings.primary_color` real del tenant en esos puntos específicos; no se barrió el resto del tema (fuera de alcance de esta card). También: `aria-label` en el logo-link del footer de servelec (sin texto accesible), 4 links de nav con `opacity-50` sobre un `rgba` ya translúcido, 2 líneas de copyright muy tenues, link de crédito sin distinción más allá del color, 3 usos de `text-slate-400` en el formulario compartido, 4 labels con gris hardcodeado en `contact.html`, y el mockup de dashboard falso de `andesscale/components/hero.html` con `aria-hidden="true"` **+** colores reales corregidos (aria-hidden no exime del contraste visual — afecta el árbol de accesibilidad, no el render; un usuario con baja visión sigue viendo el píxel). **Verificado:** los 3 specs de axe en verde, Playwright completo 7/7 (smoke+a11y), suite Django 167 tests OK sin cambios (card no tocó ningún `.py`), `makemigrations --check` limpio. **Pendiente, documentado como hallazgo no bloqueante:** solo se auditaron las 3 páginas reales servidas en `develop` — `var(--color-primary)` arbitrario por tenant puede seguir fallando AA en componentes no cubiertos acá; una guardia estática (mismo patrón que `test_theme_token_contract.py`) que exija `.text-primary-a11y`/`.bg-primary-a11y` en vez de `text-primary`/`bg-primary` crudo sobre texto sería el siguiente paso — no incluido para no ampliar un `[M]` a un rediseño del sistema de tokens.
...
- **Criterios de aceptación:**
  ...
  - [x] Ningún plan puede llamarse `process`, `success` ni `error` — **resuelto en `agent/ai-dlc-pilot` (`BOLT-02`, 2026-08-23, commit `d958064`), sin mergear a esta branch**: `Plan.clean()` rechaza los 3 slugs con `ValidationError`, `Plan.save()` llama `full_clean()`. 3 tests nuevos en `apps/orders/tests/test_models.py` (`PlanReservedSlugTestCase`).
