...
**Pivote de enfoque (2026-08-26), decisión del usuario:** se elimina el supuesto de "landing como plantilla ensamblada" que regía esta card — no más diseño top-down sobre un layout heredado (el orden de secciones de `home.html`, la reutilización estructural forzada contra `servelec`/`andesscale`/`themes/default` citada arriba) porque le quita libertad de diseño. A partir de ahora, Component-Driven Development (CDD) puro: cada archivo de `templates/ranchocachimba/components/` se diseña como lienzo aislado, sin asumir contenedores grilla ni estructura heredada de otro tema — la regla de "revisar 2+ temas antes de generalizar" queda **suspendida para Rancho Cachimba** (sigue vigente para el resto de temas/`templates/components/` compartidos). Los tokens de marca (`docs/design-system.md`: variables CSS de color, Fraunces/Inter, `.tartan`) se mantienen intactos — lo que cambia es el proceso de composición, no la identidad visual. El prompt de Stitch en `Documentacion/Planificación/prompt_stitch_home_ranchocachimba.md` (estructurado como página completa de 11 bloques en orden fijo) queda como inventario de copy/CTAs reales, no como mandato de estructura — no usar su §6 como guía de layout de aquí en adelante.
