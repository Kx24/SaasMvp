...
**Lo que sigue, sin depender de Rancho Cachimba** (todas `[P2-Media]`, independientes entre sí, sin orden forzoso — ver §5 para el detalle de cada una): `#MED-04` (mitad manual: probar en Gmail/Outlook — la mitad automatizable ya está DONE en `agent/ai-dlc-pilot`, sin mergear), `#MED-05` (DONE en `agent/ai-dlc-pilot`, sin mergear), `#SEC-03` (inventario de secretos), `#FLOW-01`/`#PAY-02` (flujo comercial repetible — `#FLOW-02` ya DONE en `agent/ai-dlc-pilot`, sin mergear). El resto (`#RC-01`/`#RC-06b`/`#RC-18`) sigue en pausa por decisión del usuario, que necesita tiempo de diseño/investigación antes de retomarlo. También quedó postergado (a pedido del usuario) verificar `EXPLAIN ANALYZE` de `#MED-03` contra la base real — nota aparte: la base de producción es **Neon**, no Supabase como dice el resto de este documento; corrección pendiente, el usuario pidió posponer el tema completo hasta tener más contexto sobre Neon.
...
Procedimiento de ingreso de cliente + `check_tenant_setup` ampliado (SEOConfig completo, sin placeholders, `ClientEmailSettings`) + registro de fricciones del cobro manual de Cachimba (es la spec de `#FLOW-03`). **`#FLOW-02` (la ampliación de `check_tenant_setup`) DONE en `agent/ai-dlc-pilot` (`BOLT-05`, 2026-08-23, commit `40ffbfa`), sin mergear:** agrega chequeo de `SEOConfig("home")` sin placeholders, secciones activas sin marcadores de relleno (`lorem`/`placeholder`/`xxx`/`TODO`), y `ClientEmailSettings` obligatorio cuando hay `FormConfig`. `#FLOW-01`/`#PAY-02` siguen abiertos.
