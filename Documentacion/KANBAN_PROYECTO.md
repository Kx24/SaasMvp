...
**`#SEC-03` cerrado en `develop` (2026-08-24, commit `5229215`).** `setup_production.py` creaba el primer superusuario de `/superadmin/` con la contraseña hardcodeada `admin123456` si `DJANGO_SUPERUSER_PASSWORD` nunca se seteó en Render (esa env var no estaba ni declarada en `render.yaml`). **Verificado por el usuario (2026-08-25) entrando a `/superadmin/` en producción: esa clave nunca llegó a estar activa ahí** — pudo haberse usado en desarrollo local, pero no en el superuser real de producción; sin impacto, cerrado sin necesidad de rotar nada. El código igual queda corregido (falla el build en vez de crear la cuenta débil) para que no pueda repetirse en un futuro re-deploy desde cero. Detalle completo y el resto del inventario de secretos (MercadoPago/Cloudinary tampoco estaban declarados en `render.yaml`) en `Documentacion/Inventario_Secretos.md` y la card `#SEC-03` de §5.
