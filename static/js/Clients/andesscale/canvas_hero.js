/**
 * AndesScale — canvas_hero.js
 * Malla de nodos dentro del hero (position: absolute).
 * Misma lógica que canvas_global pero con mayor velocidad de pulso.
 * Solo se carga en landing/home.html.
 */
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function spawnPulse() {
            const e = getEmitter();
            pulses.push({ x: e.x, y: e.y, radius: 0, speed: 2.5, maxRadius: Math.max(canvas.width, canvas.height) });
        }
            ...
            for (const node of nodes) {
                ...
                for (const p of pulses) {
                    ...
                    if (diff < 40) {
                        ...
                        if (dist !== 0) { node.x += (dx / dist) * force * 12; node.y += (dy / dist) * force * 12; }
