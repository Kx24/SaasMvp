...
Uso:
    python manage.py provision_tenant servelec-ingenieria --industry=electricidad --theme=themes/servelec
...
Nota (#FLOW-01): --industry solo controla el contenido semilla (colores,
textos de servicios). --theme controla la carpeta visual real (debe ser
uno de Client.THEME_CHOICES) y es lo único que se guarda en
Client.template. No confundir ambos flags -- antes de este fix, este
comando solo tenía --template y lo usaba para AMBAS cosas, así que el
valor guardado en Client.template nunca coincidía con ninguna carpeta
real de templates/ y el tenant caía en el fallback genérico sin marca,
sin ningún error visible (ver apps/tenants/tests_provisioning.py).
"""
