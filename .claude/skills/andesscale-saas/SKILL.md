...
> **Nota (2026-08-22):** existe un runbook manual de alta de tenant,
> `Documentacion/Procedimiento_Nuevo_Tenant.md`, pero solo vive en
> `feature/RanchocachimbaEtapa1`/`feature/mejorar-provisioning-tenant` — nunca
> llegó a `develop`/`main` — y además está desactualizado (sigue documentando
> el management command `test_isolation`, eliminado en `#MED-02`). No se cita
> acá por ruta directa para no dejar una referencia colgante en las ramas
> donde este skill sí vive; traerlo actualizado a `develop` es trabajo aparte.
