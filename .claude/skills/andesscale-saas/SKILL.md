...
> Nota de esta rama (`agent/ai-dlc-pilot`): la versión original de este skill
> citaba acá `Documentacion/Procedimiento_Nuevo_Tenant.md` (runbook manual de
> alta de tenant). Ese archivo solo existe en `feature/RanchocachimbaEtapa1`
> (nunca llegó a `develop`, base de esta rama) y además está desactualizado —
> sigue documentando el comando `test_isolation`, eliminado en `#MED-02`. Se
> quitó la cita acá para no dejar una ruta colgante; si hace falta ese runbook,
> traerlo actualizado es trabajo aparte, fuera del alcance de este piloto.
